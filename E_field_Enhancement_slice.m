...
Np_name="Ag";%input('Choose the name of the Np,\nEx. "Au" or "Ag" or "Cu" or "other" :');
clc
if Np_name=="Au"
...
E_outxy=sqrt(Ex_out.*(Ex_out)+Ey_out.*(Ey_out)+Ez_out.*(Ez_out));
...
%=========================================================================%
Et_outxy=abs(reshape(E_outxy,[SIZE(1,1),SIZE(1,2)]));
...
[Ex_out,Ey_out,Ez_out]=E_total(GPU,N_plane,Nx,Ny,Nz,INDEX_IN,Inverse_Alpha,Nx_target,...
    Ny_target,Nz_target,x_plane,y_plane,z_plane,r_block,Np_shape,plane_2D,...
    Lx,Ly,Lz,kvec,d_inter,Structure,Ex_incident,Ey_incident,Ez_incident,px,py,pz,arrangement);
E_outxz=sqrt(Ex_out.*(Ex_out)+Ey_out.*(Ey_out)+Ez_out.*(Ez_out));
Et_outxz=abs(reshape(E_outxz,[SIZE(1,1),SIZE(1,2)]));
...
[Ex_out,Ey_out,Ez_out]=E_total(GPU,N_plane,Nx,Ny,Nz,INDEX_IN,Inverse_Alpha,Nx_target,...
    Ny_target,Nz_target,x_plane,y_plane,z_plane,r_block,Np_shape,plane_2D,...
    Lx,Ly,Lz,kvec,d_inter,Structure,Ex_incident,Ey_incident,Ez_incident,px,py,pz,arrangement);
E_outyz=sqrt(Ex_out.*(Ex_out)+Ey_out.*(Ey_out)+Ez_out.*(Ez_out));
Et_outyz=abs(reshape(E_outyz,[SIZE(1,1),SIZE(1,2)]));

O=size(Et_outxy,1);
P=size(Et_outxy,2);
Q=size(Et_outyz,2);
model=zeros(O,P,Q);
Et_outxy=gather(Et_outxy);
Et_outyz=gather(Et_outyz);
Et_outxz=gather(Et_outxz);
model(:,:,ceil(Q/2))=Et_outxy;
model(ceil(O/2),:,:)=Et_outyz;
model(:,ceil(P/2),:)=Et_outxz;
[X_coor,Y_coor,Z_coor]=meshgrid(-floor(P/2):floor(P/2),-floor(O/2):floor(O/2),-floor(Q/2):floor(Q/2));
t = tiledlayout(2,2,'TileSpacing','Compact','Padding','Compact');
nexttile
slice(X_coor,Y_coor,Z_coor,model,0,0,0);
shading interp 
% colorbar 
colormap('hot') 
...
box on
alpha(0.75)
ylim([-floor(O/2),floor(O/2)])
xlim([-floor(P/2),floor(P/2)])
zlim([-floor(Q/2),floor(Q/2)])
xlabel("Y (nm)")
ylabel("X (nm)")
zlabel("Z (nm)")
title("三维视图")
